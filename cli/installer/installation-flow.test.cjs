...
test('discovery reads the existing 3X-UI runtime and never mutates it', async () => {
  ...
  assert.equal(detection.data.webPort, PANEL_PORT);
  assert.equal(detection.data.basePath, PANEL_BASE_PATH);
  assert.equal(detection.data.tlsEnabled, true);
  assert.equal(detection.data.subPort, SUB_PORT);
  assert.equal(detection.data.dbPath, PANEL_DB);
  assert.equal(detection.data.username, 'admin');
  assert.equal(detection.data.portBound, true, 'port 17342 must be observed as bound');
  assert.equal(detection.data.url, PANEL_URL);
...
test('an existing healthy panel is reused: no runtime preparation, no port change, no TLS/port/path questions', async () => {
  ...
  assert.equal(outcome.snapshot.detections.xui.data.webPort, PANEL_PORT);
  ...
  assert.equal(outcome.snapshot.detections.xui.data.subPort, SUB_PORT, 'the subscription port survives untouched');
...
test('a panel with no readable username falls back to prompting and refreshes status at once', async () => {
  ...
  assert.equal(discovery.state, STATES.NEEDS_CREDENTIALS, 'a panel without a readable username needs credentials');
