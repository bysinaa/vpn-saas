...
async function main(argv = process.argv.slice(2), deps = {}) {
  ...
  const result = await (deps.detector || createXuiDetector({ runtime: deps.runtime })).discover(options);
  ...
  else console.log(`${result.status}: ${result.connection ? result.connection.url : 'no XUI candidate'} (${result.confidence}% confidence)`);
  return result;
}

if (require.main === module) main().catch((error) => { console.error(`detect-xui: ${error.message}`); process.exitCode = 1; });
module.exports = { main, parseArgs, createXuiDetector };
