...
function createInstallationFlow({ runtime, detectors, actions = {}, prompts = {}, logger } = {}) {
  ...
  async function runPanelStep(context) {
    ...
    log(`Reusing the existing 3X-UI installation at ${detection.data.url} (port ${detection.data.webPort} left untouched).`);
    ...
    for (let attempt = 0; attempt < 3 && authenticated.state === STATES.NEEDS_CREDENTIALS; attempt += 1) {
      ...
      const supplied = await prompts.panelCredentials({ suggestedUsername: detection.data.username, attempt });
