...
class InstallCommand extends install_interface_1.BaseCommand {
    ...
    async ensureDockerInstalled(family) {
    ...
    async ensureDockerComposeInstalled(family) {
    ...
    async ensureBasePackages(family) {
    ...
    async configureFirewall(httpPort, httpsPort, apiPort) {
    ...
    async ensure3xuiRuntime(options, publicIp) {
    ...
    async ensureEnvironmentWizard(options, publicIp, apiPort, panel) {
    ...
    async promptForValidTelegramBotToken() {
        while (true) {
    ...
    assertEnvHasValues(content, keys) {
