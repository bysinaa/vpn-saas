...
export class InfrastructureCommand extends BaseCommand {
  ...
  private async detectPostgreSQL(): Promise<DetectionResult> {
    ...
    const detected = result.status !== 'NOT_FOUND';
    this.log(detected ? `Detected PostgreSQL via ${result.source}` : 'PostgreSQL was not discovered', result.status === 'FOUND' ? 'success' : 'warn');
