...
export class InstallCommand extends BaseCommand {
  private readonly XUI_REPO = 'mhsanaei/3x-ui';
  private readonly XUI_TAG = 'v2.3.2';
  private readonly XUI_INSTALL_URL = `https://raw.githubusercontent.com/${this.XUI_REPO}/${this.XUI_TAG}/install.sh`;
  ...
  private async check3xuiInstalled(): Promise<boolean> {
  ...
  private async get3xuiStatus(): Promise<{ running: boolean; port: number; subPath: string; subPort: number }> {
  ...
  private async handleExisting3xui(options: InstallOptions): Promise<void> {
  ...
  private async handleNew3xui(options: InstallOptions): Promise<void> {
  ...
  private async configure3xui(options: InstallOptions): Promise<void> {
  ...
  private async configureBot(options: InstallOptions): Promise<void> {
  ...
  private async showFinalInstructions(): Promise<void> {
    console.log('\n' + '═'.repeat(50));
  ...
  private async syncExistingUsers(url: string, user: string, pass: string): Promise<void> {
  ...
  private async savePanelConfig(config: { url: string; user: string; pass: string; subPort: number; subPath: string }): Promise<void> {
  ...
  private generateEnvFile(config: { botToken: string; databaseUrl: string; redisUrl: string; serverIp: string }): string {
