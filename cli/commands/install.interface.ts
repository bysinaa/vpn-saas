...
const { composeCommand: buildComposeCommand } = require('../installer/compose-lifecycle') as {
...
export abstract class BaseCommand {
  ...
  protected composeCommand(subcommand: string): string {
    return buildComposeCommand({
      composeFile: path.join(this.workspaceRoot, 'docker-compose.yml'),
  ...
  protected async execWithInput(cmd: string, input: string, options: ExecOptions = {}): Promise<ExecResult> {
    return new Promise((resolve) => {
      ...
      child.on('error', (error) => {
        clearTimeout(timer);
        resolve({ stdout, stderr: error.message, exitCode: 1, ok: false });
      });
      child.on('close', (code) => {
        clearTimeout(timer);
