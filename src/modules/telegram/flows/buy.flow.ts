 ...
 *
 * Payment methods:
 *  - WALLET     : instant debit + provision (PaymentsService.payOrderWithWallet)
...
export class BuyFlow {
  ...
  private async payWithWallet(ctx: Context, orderPublicId: string): Promise<void> {
    ...
    const session = await this.runtime.getSession(telegramId);
    try {
      const result = await this.payments.payOrderWithWallet(orderPublicId, session.userId!);
      const link =
        result.subscription?.subscriptionLink ?? result.subscription?.vpnUser?.subLink ?? null;
      await this.runtime.clearState(telegramId);
      await this.runtime.resetMenu(telegramId, 'main');
      await this.runtime.alert(ctx);
      await this.runtime.send(
        ctx,
        `${t(locale, 'pay.wallet.success')}\n\n${result.subscription.plan?.name ?? ''}\n\`${link}\``,
        mainMenuKeyboard(locale),
        'Markdown',
      );
      ...
      const order = session.userId
        ? await this.orders.findOne(orderPublicId, session.userId).catch(() => null)
        : null;
      await this.runtime.alert(ctx);
      await this.runtime.render(
        ctx,
        order?.status === 'COMPLETED'
          ? t(locale, 'pay.provisioning.pending')
          : this.runtime.translateError(locale, err),
        mainMenuKeyboard(locale),
      );
    }
      ...
      await this.runtime.render(
        ...
        `${t(locale, 'pay.crypto.title')}\n\n` +
          t(locale, 'pay.crypto.instructions', {
            amount: order.totalAmount,
            currency: payment.cryptoNetwork
              ? `${cryptoCurrency} (${payment.cryptoNetwork})`
              : cryptoCurrency,
  ...
  async onReceiptUpload(ctx: Context, photoFileId: string): Promise<void> {
    ...
    try {
      ...
      await this.runtime.notifyAdminsWithPhoto({
        fileId: photoFileId,
