...
export class BotRuntime {
  ...
  async notifyAdmins(payload: {
    ...
    await Promise.all(
      (await this.getAdminTelegramIds()).map(async (adminId) => {
        try {
  ...
  /** Send one independent receipt notification to every authorized admin. */
  async notifyAdminsWithPhoto(payload: {
    fileId: string;
    caption: string;
    keyboard: any;
  }): Promise<void> {
    ...
    const extra = { reply_markup: payload.keyboard?.reply_markup ?? payload.keyboard };
    await Promise.all(
      (await this.getAdminTelegramIds()).map(async (adminId) => {
        try {
          await this._bot.telegram.sendPhoto(adminId, payload.fileId, {
            caption: payload.caption,
            ...extra,
          });
        } catch (photoError: any) {
          try {
            await this._bot.telegram.sendMessage(adminId, payload.caption, extra);
          } catch (textError: any) {
            this.logger.error(
              `Receipt notification failed for admin ${adminId}: ${textError?.message ?? photoError?.message}`,
            );
          }
        }
      }),
  ...
  private async getAdminTelegramIds(): Promise<string[]> {
    ...
    return [
      ...new Set(
        [...config.telegram.adminIds, config.superAdmin.telegramId, ...users.map((u) => u.telegramId)]
