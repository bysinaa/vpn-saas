...
export class TelegramBotService implements OnModuleInit, OnApplicationBootstrap, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    if (process.env.TAZAXY_CLI_CONTEXT === '1') {
      this.logger.log('Telegram bot launch disabled for CLI application context');
      return;
    }
  ...
  private registerHandlers(): void {
    this.bot.use(this.mandatoryJoinGuard.bind(this));
    ...
    this.bot.action('joincheck', this.onJoinCheck.bind(this));
    ...
    this.bot.action('avoucher:new', (ctx) => this.admin.onVoucherAction(ctx, 'new'));
    this.bot.action(/avoucher:plan:([0-9a-f-]+)/, (ctx) =>
      this.admin.onVoucherAction(ctx, 'plan', this.match(ctx, /avoucher:plan:([0-9a-f-]+)/)!),
    );
    ...
    this.bot.action(/acard:(detail|toggle|default|confirmDelete|delete):([0-9a-f-]+)/, (ctx) => {
      const m = (ctx.callbackQuery as any)?.data?.match(
        /acard:(detail|toggle|default|confirmDelete|delete):([0-9a-f-]+)/,
      );
      return this.admin.onCardAction(ctx, m![1], m![2]);
    });
    this.bot.action('acard:new:0', (ctx) => this.admin.onCardAction(ctx, 'new', ''));
    this.bot.action(/acardedit:(number|holder|bank|sheba|label):([0-9a-f-]+)/, (ctx) => {
      const m = (ctx.callbackQuery as any)?.data?.match(
        /acardedit:(number|holder|bank|sheba|label):([0-9a-f-]+)/,
      );
      return this.admin.onCardEditField(ctx, m![1], m![2]);
    });
    this.bot.action(/acrypto:(detail|toggle|default|confirmDelete|delete):([0-9a-f-]+)/, (ctx) => {
      const m = (ctx.callbackQuery as any)?.data?.match(
        /acrypto:(detail|toggle|default|confirmDelete|delete):([0-9a-f-]+)/,
      );
      return this.admin.onCryptoAction(ctx, m![1], m![2]);
    });
    this.bot.action('acrypto:new:0', (ctx) => this.admin.onCryptoAction(ctx, 'new', ''));
    this.bot.action(/acrypto:choose:(USDT_TRC20|USDT_ERC20|TON|BTC|ETH)/, (ctx) => {
      const m = (ctx.callbackQuery as any)?.data?.match(
        /acrypto:choose:(USDT_TRC20|USDT_ERC20|TON|BTC|ETH)/,
      );
      return this.admin.onCryptoAction(ctx, 'choose', m![1]);
    });
    this.bot.action(/acryptoedit:(address|network|label|instructions):([0-9a-f-]+)/, (ctx) => {
      const m = (ctx.callbackQuery as any)?.data?.match(
        /acryptoedit:(address|network|label|instructions):([0-9a-f-]+)/,
      );
      return this.admin.onCryptoEditField(ctx, m![1], m![2]);
    });
    ...
    this.bot.action(/ajoin:(toggle|add|remove)(?::(\d+))?/, (ctx) => {
      ...
      return this.admin.onMandatoryJoinAction(ctx, match![1], match![2] ?? '');
    });
    this.bot.action(/apm:cancel:(cards|crypto|gateway)/, (ctx) =>
      this.admin.onPaymentWizardCancel(
        ctx,
        this.match(ctx, /apm:cancel:(cards|crypto|gateway)/)! as 'cards' | 'crypto' | 'gateway',
      ),
    );
    ...
    this.bot.action(/paymethod:(.+)/, (ctx) =>
      this.buy.onSelectPaymentMethod(
        ctx,
        this.match(ctx, /paymethod:(.+)/)! as 'WALLET' | 'ONLINE' | 'CARD_TO_CARD' | 'CRYPTO',
      ),
    );
  ...
  private async mandatoryJoinGuard(ctx: Context, next: () => Promise<void>): Promise<void> {
    const telegramId = this.getTelegramId(ctx);
    if (!telegramId || (ctx.callbackQuery as any)?.data === 'joincheck') return next();
    if (await this.isAdminTelegramUser(telegramId)) return next();
    const enabled = await this.settings.getValue<boolean>('telegram.mandatoryJoin.enabled', false);
    const channels = await this.settings.getValue<MandatoryJoinChannel[]>(
      'telegram.mandatoryJoin.channels',
      [],
    );
    if (
      !enabled ||
      !channels.length ||
      !(await this.missingJoinChannels(ctx, telegramId, channels)).length
    ) {
      return next();
    }
    const messageText = (ctx.message as { text?: string } | undefined)?.text;
    const startPayload = (ctx as any).startPayload ?? messageText?.split(' ')[1];
    if (startPayload) {
      await this.runtime.setState(telegramId, 'idle', { pendingStartPayload: startPayload });
    }
    await this.renderJoinRequired(ctx, channels);
  }

  private async onJoinCheck(ctx: Context): Promise<void> {
    const telegramId = this.getTelegramId(ctx);
    if (!telegramId) return;
    const channels = await this.settings.getValue<MandatoryJoinChannel[]>(
      'telegram.mandatoryJoin.channels',
    ...
    const missing = await this.missingJoinChannels(ctx, telegramId, channels);
    await ctx
      .answerCbQuery(
        missing.length ? 'هنوز عضویت شما در همه کانال‌ها تأیید نشده است.' : 'عضویت تأیید شد.',
      )
      .catch(() => {});
    if (missing.length) return this.renderJoinRequired(ctx, missing);
    await this.onStart(ctx);
  }

  private async missingJoinChannels(
    ctx: Context,
    telegramId: string,
    channels: MandatoryJoinChannel[],
  ): Promise<MandatoryJoinChannel[]> {
    const checks = await Promise.all(
      channels.map(async (channel) => {
        try {
          const member = await ctx.telegram.getChatMember(channel.chatId, Number(telegramId));
          return ['creator', 'administrator', 'member'].includes(member.status) ? null : channel;
        } catch {
          return channel;
        }
      }),
    );
    return checks.filter((channel): channel is MandatoryJoinChannel => Boolean(channel));
  }

  private async renderJoinRequired(ctx: Context, channels: MandatoryJoinChannel[]): Promise<void> {
    const keyboard = Markup.inlineKeyboard([
      ...channels.map((channel) => [
        Markup.button.url(`📢 عضویت در ${channel.title}`, channel.url),
      ]),
      [Markup.button.callback('✅ عضو شدم', 'joincheck')],
    ]);
    await this.runtime.editOrSend(
      ctx,
      '🔒 برای استفاده از ربات ابتدا در کانال‌های زیر عضو شوید، سپس روی «عضو شدم» بزنید.',
      keyboard,
    );
  }

  private async isAdminTelegramUser(telegramId: string): Promise<boolean> {
    const normalize = (value?: string | null) => value?.trim().replace(/^\+/, '') ?? '';
    if (normalize(config.superAdmin.telegramId) === normalize(telegramId)) return true;
  ...
  private async ensureUser(ctx: Context): Promise<void> {
    ...
    const rawPayload =
      ...
      session.data?.pendingStartPayload;
  ...
  private async showMenu(ctx: Context): Promise<void> {
    ...
    if (!telegramId) return;
    const locale = await this.runtime.getLocale(telegramId);
    const keyboard = await this.runtime.getMainMenuKeyboard(telegramId);
    await this.runtime.resetMenu(telegramId, 'main');
    await this.runtime.setState(telegramId, 'idle');
    // Spec #7 UX: edit the existing message in place (no new message on menu tap).
    await this.runtime.editOrSend(ctx, t(locale, 'menu.title'), keyboard);
  }
  ...
  private async onText(ctx: any): Promise<void> {
    ...
    if (text) {
      ...
      // Admin CRUD wizards (spec #9/#10 — plans/settings/payment methods text input)
      if (
