...
export class CryptoWalletsService {
  ...
  async findOne(publicId: string): Promise<CryptoWalletDto> {
    const wallet = await this.prisma.cryptoWallet.findUnique({ where: { publicId } });
    if (!wallet) throw BusinessException.notFound('Crypto wallet not found');
  ...
  async create(
    ...
    const wallet = await this.prisma.cryptoWallet.create({
      data: {
        ...
        address: this.checkedAddress(input.address),
  ...
  async update(
    ...
    if (input.address !== undefined) data.address = this.checkedAddress(input.address);
  ...
  async setActive(publicId: string, isActive: boolean, adminId?: bigint): Promise<CryptoWalletDto> {
    return this.update(publicId, { isActive, ...(isActive ? {} : { isDefault: false }) }, adminId);
  }

  async setDefault(publicId: string, adminId?: bigint): Promise<CryptoWalletDto> {
    const wallet = await this.prisma.cryptoWallet.findUnique({ where: { publicId } });
    if (!wallet) throw BusinessException.notFound('Crypto wallet not found');
    if (!wallet.isActive) throw BusinessException.conflict('Only an active wallet can be default');
    await this.clearDefault(wallet.currency);
    return this.update(publicId, { isDefault: true }, adminId);
  }

  private checkedAddress(value: string): string {
    const address = value.trim();
    if (address.length < 10 || address.length > 200) {
      throw new BusinessException('VALIDATION_ERROR', 'Invalid wallet address');
    }
    return address;
  }
