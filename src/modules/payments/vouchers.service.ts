 ...
 *   4. Records one immutable redemption row per user and increments usage
 *   5. After commit, enqueues XUI panel user creation via VpnService
 *   6. Audit-logs the REDEEM action
 *
 * A code may be multi-use, but each user can redeem that code only once.
 */
@Injectable()
export class VouchersService {
  ...
  async generate(
    ...
    if (
      ...
      throw BusinessException.conflict('Voucher usage limit must be a positive integer');
    }
  ...
  async redeem(
    ...
    const { subscription, updatedVoucher } = await this.prisma.withTransaction(async (tx) => {
      await tx.$queryRaw`SELECT id FROM vouchers WHERE id = ${voucher.id} FOR UPDATE`;
      const fresh = await tx.voucher.findUnique({
        where: { id: voucher.id },
        select: { redemptions: true, maxRedemptions: true, isActive: true, expiresAt: true },
      });
      ...
      if (fresh.redemptions >= fresh.maxRedemptions) {
        throw new BusinessException('VOUCHER_INVALID', 'Voucher redemption limit reached');
      }
      const priorUse = await tx.voucherRedemption.findUnique({
        where: { voucherId_userId: { voucherId: voucher.id, userId: params.userId } },
        select: { id: true },
      });
      if (priorUse) {
        throw new BusinessException('VOUCHER_INVALID', 'You have already used this voucher');
      }
      ...
      const activation = {
        ...
        voucherPublicId: voucher.publicId,
      ...
      const exhausted = newRedemptions >= fresh.maxRedemptions;
    ...
    // After commit: provisioning failures are persisted and returned to prevent false delivery.
    ...
    await this.invalidate();
    this.logger.log(
      `Voucher redeemed by user ${params.userId} (telegram=${params.telegramId}) → subscription ${subscription.id}`,
    );
