...
export class BankCardsService {
  ...
  async create(
    ...
    const card = await this.prisma.bankCard.create({
      data: {
        ...
        cardHolder: this.checkedText(input.cardHolder, 'Card holder'),
        bankName: this.checkedText(input.bankName, 'Bank name'),
        shebaNumber: this.checkedSheba(input.shebaNumber),
  ...
  async update(
    ...
    if (input.cardHolder !== undefined) data.cardHolder = this.checkedText(input.cardHolder, 'Card holder');
    if (input.bankName !== undefined) data.bankName = this.checkedText(input.bankName, 'Bank name');
    if (input.shebaNumber !== undefined) data.shebaNumber = this.checkedSheba(input.shebaNumber);
  ...
  private checkedText(value: string, field: string): string {
    const normalized = value.trim();
    if (!normalized) throw new BusinessException('VALIDATION_ERROR', `${field} is required`);
    return normalized;
  }

  private checkedSheba(value?: string | null): string | null {
    if (value == null) return null;
    const normalized = value.replace(/[\s-]/g, '').toUpperCase();
    if (!/^(IR)?\d{16,32}$/.test(normalized)) {
      throw new BusinessException('VALIDATION_ERROR', 'Invalid sheba number');
    }
    return normalized;
  }
