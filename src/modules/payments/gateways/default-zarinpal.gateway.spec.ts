...
describe('DefaultZarinpalGateway official v4 contract', () => {
  ...
  const settings = {
    getValue: jest.fn(async (_key: string, fallback: unknown) => fallback),
  };
  const gateway = new DefaultZarinpalGateway(proxy as any, settings as any);

  beforeEach(() => {
    jest.clearAllMocks();
    settings.getValue.mockImplementation(async (_key: string, fallback: unknown) => fallback);
  ...
  it('uses runtime settings and blocks new requests when disabled', async () => {
    settings.getValue.mockImplementation(async (key: string, fallback: unknown) => {
      if (key === 'gateway.default.enabled') return false;
      return fallback;
    });
