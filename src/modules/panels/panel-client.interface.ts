...
export function buildSubscriptionUrl(
  panel: Pick<PanelConnection, 'baseUrl' | 'subPort' | 'subPath' | 'extraConfig'>,
  subId: string,
): string {
  const publicBase = panel.extraConfig?.publicSubscriptionBaseUrl;
  const url = new URL(typeof publicBase === 'string' ? publicBase : panel.baseUrl);
  if (typeof publicBase !== 'string' && panel.subPort != null) url.port = String(panel.subPort);
  const path = (panel.subPath || 'sub').replace(/^\/+|\/+$/g, '');
  url.pathname = `/${path}/${encodeURIComponent(subId)}`;
  url.search = '';
  url.hash = '';
  return url.toString();
}
