...
export class XuiPanelClient implements IPanelClient {
  ...
  async getClient(panel: PanelConnection, email: string): Promise<XuiClientState | null> {
    ...
    return this.api(
      panel,
      `/panel/api/clients/get/${encodeURIComponent(safeEmail)}`,
      'GET',
      undefined,
      clientGetSchema.nullable(),
    );
  }

  async addClient(
    panel: PanelConnection,
    client: XuiClientState,
    inboundIds?: number[],
  ): Promise<void> {
    const state = clientStateSchema.parse(client);
    const body = inboundIds
      ? { client: state, inboundIds: inboundIdsSchema.parse(inboundIds) }
      : { client: state };
  ...
  async replaceClient(
    ...
    const safeEmail = emailSchema.parse(email);
    const state = clientStateSchema.parse(client);
    if (state.email !== safeEmail)
      throw BusinessException.conflict('Client email cannot change during replacement');
    await this.api(
      panel,
      `/panel/api/clients/update/${encodeURIComponent(safeEmail)}`,
      'POST',
      state,
      z.unknown(),
    );
  ...
  async deleteClient(panel: PanelConnection, email: string): Promise<void> {
    await this.api(
      panel,
      `/panel/api/clients/del/${encodeURIComponent(emailSchema.parse(email))}`,
      'POST',
      undefined,
      z.unknown(),
    );
  ...
  async attachClient(panel: PanelConnection, email: string, inboundIds: number[]): Promise<void> {
    await this.clientAction(panel, email, 'attach', {
      inboundIds: inboundIdsSchema.parse(inboundIds),
    });
  }

  async detachClient(panel: PanelConnection, email: string, inboundIds: number[]): Promise<void> {
    await this.clientAction(panel, email, 'detach', {
      inboundIds: inboundIdsSchema.parse(inboundIds),
    });
  }
  ...
  async subscriptionLinks(panel: PanelConnection, subId: string): Promise<string[]> {
    return this.api(
      panel,
      `/panel/api/clients/subLinks/${encodeURIComponent(subIdSchema.parse(subId))}`,
      'GET',
      undefined,
      this.linksSchema(),
    );
  ...
  async clientLinks(panel: PanelConnection, email: string): Promise<string[]> {
    return this.api(
      panel,
      `/panel/api/clients/links/${encodeURIComponent(emailSchema.parse(email))}`,
      'GET',
      undefined,
      this.linksSchema(),
    );
  ...
  async bulkCreate(panel: PanelConnection, clients: XuiClientState[]): Promise<XuiBulkResult> {
    return this.bulk(panel, 'bulkCreate', {
      clients: z.array(clientStateSchema).min(1).parse(clients),
    });
  }
  ...
  async bulkAdjust(
    ...
    trafficBytes?: number,
    expiryMs?: number,
  ): Promise<XuiBulkResult> {
    return this.bulk(panel, 'bulkAdjust', {
      emails: this.emails(emails),
      ...(trafficBytes === undefined ? {} : { totalGB: bytesSchema.parse(trafficBytes) }),
      ...(expiryMs === undefined ? {} : { expiryTime: expiryMsSchema.parse(expiryMs) }),
    });
  }

  async bulkEnable(panel: PanelConnection, emails: string[]): Promise<XuiBulkResult> {
    return this.bulk(panel, 'bulkEnable', { emails: this.emails(emails) });
  }
  async bulkDisable(panel: PanelConnection, emails: string[]): Promise<XuiBulkResult> {
    return this.bulk(panel, 'bulkDisable', { emails: this.emails(emails) });
  }
  async bulkDelete(panel: PanelConnection, emails: string[]): Promise<XuiBulkResult> {
    return this.bulk(panel, 'bulkDel', { emails: this.emails(emails) });
  }
  async bulkAttach(
    panel: PanelConnection,
    emails: string[],
    ...
    return this.bulk(panel, 'bulkAttach', {
      emails: this.emails(emails),
      inboundIds: inboundIdsSchema.parse(inboundIds),
    });
  }
  async bulkDetach(
    ...
    return this.bulk(panel, 'bulkDetach', {
      emails: this.emails(emails),
      inboundIds: inboundIdsSchema.parse(inboundIds),
    });
  }
  async bulkResetTraffic(panel: PanelConnection, emails: string[]): Promise<XuiBulkResult> {
    return this.bulk(panel, 'bulkResetTraffic', { emails: this.emails(emails) });
  }

  async listGroups(panel: PanelConnection): Promise<string[]> {
    return this.api(panel, '/panel/api/groups/list', 'GET', undefined, z.array(z.string()));
  }
  async groupEmails(panel: PanelConnection, group: string): Promise<string[]> {
    return this.api(
      panel,
      `/panel/api/groups/emails/${encodeURIComponent(subIdSchema.parse(group))}`,
      'GET',
      undefined,
      z.array(emailSchema),
    );
  }
  async createGroup(panel: PanelConnection, group: string): Promise<void> {
    await this.api(
      panel,
      '/panel/api/groups/create',
      'POST',
      { name: subIdSchema.parse(group) },
      z.unknown(),
    );
  }
  async renameGroup(panel: PanelConnection, group: string, name: string): Promise<void> {
    await this.api(
      panel,
      '/panel/api/groups/rename',
      'POST',
      { group: subIdSchema.parse(group), name: subIdSchema.parse(name) },
      z.unknown(),
    );
  }
  async deleteGroup(panel: PanelConnection, group: string): Promise<void> {
    await this.api(
      panel,
      '/panel/api/groups/del',
      'POST',
      { group: subIdSchema.parse(group) },
      z.unknown(),
    );
  }
  async bulkAddGroup(panel: PanelConnection, group: string, emails: string[]): Promise<void> {
    await this.api(
      panel,
      '/panel/api/groups/bulkAdd',
      'POST',
      { group: subIdSchema.parse(group), emails: this.emails(emails) },
      z.unknown(),
    );
  }
  async bulkRemoveGroup(panel: PanelConnection, group: string, emails: string[]): Promise<void> {
    await this.api(
      panel,
      '/panel/api/groups/bulkRemove',
      'POST',
      { group: subIdSchema.parse(group), emails: this.emails(emails) },
      z.unknown(),
    );
  ...
  async onlines(panel: PanelConnection): Promise<Record<string, unknown>> {
    return this.api(panel, '/panel/api/clients/onlines', 'GET', undefined, z.record(z.unknown()));
  }
  async onlinesByGuid(panel: PanelConnection): Promise<Record<string, unknown>> {
    return this.api(
      panel,
  ...
  async clientIpsByGuid(panel: PanelConnection): Promise<Record<string, unknown>> {
    return this.api(
      panel,
  ...
  async activeInbounds(panel: PanelConnection): Promise<Record<string, unknown>> {
    return this.api(
      panel,
  ...
  async lastOnline(panel: PanelConnection): Promise<Record<string, unknown>> {
    return this.api(
      panel,
  ...
  async exportClients(panel: PanelConnection): Promise<Record<string, unknown>> {
    return this.api(panel, '/panel/api/clients/export', 'GET', undefined, z.record(z.unknown()));
  }

  async resetAllTraffics(panel: PanelConnection): Promise<void> {
    await this.api(panel, '/panel/api/clients/resetAllTraffics', 'POST', undefined, z.unknown());
  }
  async deleteDepleted(panel: PanelConnection): Promise<void> {
    await this.api(panel, '/panel/api/clients/delDepleted', 'POST', undefined, z.unknown());
  }
  async deleteOrphans(panel: PanelConnection): Promise<void> {
    await this.api(panel, '/panel/api/clients/delOrphans', 'POST', undefined, z.unknown());
  }
  async importClients(panel: PanelConnection, payload: Record<string, unknown>): Promise<void> {
    await this.api(panel, '/panel/api/clients/import', 'POST', payload, z.unknown());
  }
  ...
  async deleteUser(panel: PanelConnection, username: string): Promise<void> {
    await this.deleteClient(panel, username);
  }
  ...
  private async request<T>(
    ...
    while (true) {
      ...
      try {
        ...
        throw BusinessException.conflict('3x-ui request error');
      }
      clearTimeout(timeout);
  ...
  private async login(panel: PanelConnection): Promise<PanelSession> {
    ...
    const storedCredentials =
      separator > 0
        ? {
            username: panel.apiKey.slice(0, separator),
            password: panel.apiKey.slice(separator + 1),
          }
        : null;
    const username =
      storedCredentials?.username ??
      (panel.extraConfig?.username as string | undefined) ??
      config.xui.username;
    const password =
      storedCredentials?.password ??
      (panel.extraConfig?.password as string | undefined) ??
      config.xui.password;
  ...
  private async createUserFromClientApi(
    ...
    const client: XuiClientState = {
      ...
      ...(input.telegramId &&
      /^\d+$/.test(input.telegramId) &&
      Number.isSafeInteger(Number(input.telegramId))
    ...
    return created
      ? this.toPanelUser(created, panel)
      : {
          uuid: '',
  ...
  private async updateUserFromClientApi(
    ...
    const replacement: XuiClientState = {
      ...existing,
      enable:
        input.status === 'disabled' ? false : input.status === 'active' ? true : existing.enable,
      totalGB:
      ...
      expiryTime:
        input.expireMs === undefined
          ? existing.expiryTime
          : input.expireMs === null
            ? 0
            : expiryMsSchema.parse(input.expireMs),
      limitIp:
        input.deviceLimit === undefined
          ? existing.limitIp
          : z.number().int().nonnegative().parse(input.deviceLimit),
      subId: input.subId === undefined ? existing.subId : subIdSchema.parse(input.subId),
      tgId:
        input.telegramId &&
        /^\d+$/.test(input.telegramId) &&
        Number.isSafeInteger(Number(input.telegramId))
  ...
  private async api<T>(
    ...
    if (!envelope.success)
      throw BusinessException.conflict('XUI returned a malformed response envelope');
    if (!envelope.data.success) {
      const message = envelope.data.msg || 'XUI rejected the request';
      if (/duplicate|already exists/i.test(message))
        throw BusinessException.conflict('XUI client already exists');
      throw BusinessException.conflict(message);
    }
    const parsed = schema.safeParse(envelope.data.obj);
    if (!parsed.success)
      throw BusinessException.conflict('XUI returned an invalid response payload');
    return parsed.data;
  }
  ...
  private async clientAction<T = void>(
    ...
    const resultSchema =
      schema ?? (z.unknown().transform(() => undefined) as unknown as z.ZodType<T>);
    return this.api(
      panel,
    ...
    return this.api(
      ...
      z
        .object({
          ...
          skipped: z.array(z.object({ email: emailSchema, reason: z.string() })).optional(),
        })
        .passthrough(),
    );
  }
  ...
  private linksSchema(): z.ZodType<string[]> {
    return z
      .union([z.array(z.string()), z.object({ links: z.array(z.string()) })])
      .transform((value) => (Array.isArray(value) ? value : value.links)) as unknown as z.ZodType<
      string[]
    >;
  }
  ...
  private toPanelUser(client: XuiClientState, panel: PanelConnection): PanelUser {
    const subLink = client.subId ? buildSubscriptionUrl(panel, client.subId) : '';
    return {
      uuid: client.uuid ?? '',
      username: client.email,
  ...
  private mapUser(
    ...
    const subLink = raw.subId ? buildSubscriptionUrl(panel, raw.subId) : '';
