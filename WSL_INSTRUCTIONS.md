...
4) If you are satisfied, run the installer as root:
   ...
   sudo VPN_SAAS_BRANCH=main VPN_SAAS_INSTALL_DIR=/opt/vpn-saas \
     VPN_SAAS_REPO_URL=https://github.com/bysinaa/vpn-saas.git bash /tmp/vpn-install.sh
...
- The installer will install packages, clone/update /opt/vpn-saas, build the CLI and invoke the interactive install flow. Run on a controlled host.
...
- This runs the one-liner inside your default WSL distro:
  wsl -- bash -lc "curl -Ls https://raw.githubusercontent.com/bysinaa/vpn-saas/main/scripts/install.sh | sudo bash -s --"
- If your distro name is different (e.g., Ubuntu-22.04), use:
  wsl -d Ubuntu-22.04 -- bash -lc "curl -Ls https://raw.githubusercontent.com/bysinaa/vpn-saas/main/scripts/install.sh | sudo bash -s --"
